...
decimals; by `it12` it has been offered **780** wall cliques, uses 17 of them, and they carry
`0.470` in total, with `ȳ(K) = 1.0099` against `ȳ(P_p) = 0.9911` — an extra of `0.019`, where the
interior family's is `0.084` on the same LP's dual (§2).  That the wall family's whole contribution
stays at `0.002` with 780 columns offered is precisely `ANCHOR.md` §4's "a clique column costs the
same as the point column it contains, so it only pays on a strip of width `eps`" — true of the
wall family, and the reason it is the wrong family.  Its LP value has come down from `12.28` to `12.077` while the wall run's sits at
...
| run | what | reached |
|---|---|---|
| `runs/branch_J6.log` | pure cover at `3.99`, **wall** separator (the control) | `it12`, LP `12.232`, probe `0.931`, gain `+0.0023` |
