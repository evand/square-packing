...
predates those options; it is pinned by `certificates/SHA256SUMS`, and what it asserts
...
By LP duality the total weight of any certificate at container side `s` is at least the
**fractional packing number** `ν_f(s)`, so the method can prove `s(12) ≥ s` only where
`ν_f(s) < 12`.  Numerically that limit is close to the bound here: column generation at
`3.9696` no longer gets below 12 (`search/TIGHTEN.md`), and `search/CEILING.md` has heuristic
estimates of where `ν_f` crosses 12, with what is and is not rigorous about them.  Exactly where
the limit lies is not settled here.  Closing the remaining gap to 4 needs case analysis
